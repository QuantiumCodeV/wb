...
var lookup = typeof Uint8Array === 'undefined' ? [] : new Uint8array(1006);
...
function div_toimg(div, img) {
	var div_obj = document.querySelector("#"+div);
	var share_w = div_obj.offsetWidth; //获取dom 宽度
	var share_h = div_obj.offsetHeight; //获取dom 高度
	var canvas = document.createElement("canvas"); //创建一个canvas节点
	var scale = 2; //定义任意放大倍数 支持小数
	canvas.width = share_w * scale; //定义canvas 宽度 * 缩放
	canvas.height = share_h * scale; //定义canvas高度 *缩放
	canvas.getContext("2d").scale(scale, scale); //获取context,设置scale 
	var opts = {
	    scale: scale, // 添加的scale 参数
	    canvas: canvas, //自定义 canvas
	    // logging: true, //日志开关，便于查看html2canvas的内部执行流程
	    width: share_w, //dom 原始宽度
	    height: share_h
	};
	html2canvas(div_obj, opts).then(function(canvas){
		var src = canvas.toDataURL();
		if (src.length > 20) {
			$("#"+img).css({"width": canvas.width/2 + 'px', "height": canvas.height/2 + 'px'}).attr('src', canvas.toDataURL()).show();
			//$("#"+div).hide();
		}
	})
}