...
function ckplayerConfig() {
    return {
        ...
        style: {
            //风格部分内容配置，这里主要配置loading和logo以及广告的部分内容
            loading: {
                //视频加载前显示在播放器中间的图片，如果不需要可以删除该属性
            ...
            video: {
                //视频的默认比例，当视频元数据里没有宽和高属性时默认使用该宽高
                defaultWidth: 16, //宽度
                defaultHeight: 9, //高度
            },
        },
    ...
    var ckplayer = function (obj) {
		...
		*/
        ...
        this.language = {
            ...
            adLink: "查看详情",
            noLoadShockwaveFlash:
                "您的浏览器不支持FlashPlayer插件或没有启用该插件",
        ...
        //全局变量：设置字体
        this.fontFamily =
            '"Microsoft YaHei"; YaHei; "FAE8F6F96C59ED1"; SimHei; "9ED1F53";Arial';
        //全局变量：设置字幕的文字大小
    ...
    ckplayer.prototype = {
        ...
        embed: function (c) {
            ...
            if (typeof videoString == "object") {
                if (videoString.length > 1) {
                    if (
                        videoString[0][0].substr(0, 3) == "CK:" ||
                        videoString[0][0].substr(0, 3) == "CE:" ||
                        videoString[0][0].substr(8, 3) == "CK:" ||
                        videoString[0][0].substr(8, 3) == "CE:"
                    ) {
                        this.vars["flashplayer"] = true;
                    }
                }
		...
		*/
        getHtml5Video: function () {
            ...
            var supportType = function (vidType, codType) {
                ...
                var isSupp = video.canPlayType(
                    vidType + ';codecs="' + codType + '"'
                );
                if (isSupp == "") {
                    return false;
                }
            ...
            if (this.html5Video) {
                ...
                try {
                    ...
                    if (
                        ...
                        if (
                            this.vars["playbackrate"] <
                            this.playbackRateArr.length
                        ) {
        ...
        adAnalysis: function () {
            ...
            if (
                ...
                if (!this.isUndefined(thisTemp.advertisements["inserttime"])) {
                    ...
                    for (
                        i = 0;
                        i < thisTemp.advertisements["inserttime"].length;
                        i++
                    ) {
                        isInsert.push(false);
                    }
		...
		*/
        addVEvent: function () {
            ...
            var eventJudgeIsLive = function (event) {
                ...
                if (
                    typeof thisTemp.V.duration == "number" &&
            ...
            var eventTimeupdate = function (event) {
                ...
                if (thisTemp.time) {
                    if (!thisTemp.adPlayerPlay) {
                        ...
                        if (
                            !thisTemp.isUndefined(
                                thisTemp.advertisements["insert"]
                            )
                        ) {
            ...
            var eventVolumeChange = function (event) {
                try {
                    ...
                    thisTemp.sendJS(
                        "volume",
                        thisTemp.volume || thisTemp.V.volume
                    );
                } catch (event) {}
            };
		...
		*/
        ...
        interFace: function () {
            ...
            var timeInto =
                this.formatTime(0) +
                " / " +
                this.formatTime(this.vars["duration"]); //时间显示框默认显示内容
			...
			*/
            ...
            this.getByElement(timeBOBGID).innerHTML =
                ...
                '"></div></div>';
            //构建进度条内容结束
            this.getByElement(pauseCenterID).innerHTML = this.newCanvas(
                pauseCenterID,
                80,
                80
            ); //构建中间暂停按钮
            this.getByElement(loadingID).innerHTML = this.newCanvas(
                loadingID,
                60,
                60
            ); //构建中间缓冲时显示的图标
            ...
            this.getByElement(adPauseCloseID).innerHTML = this.newCanvas(
                adPauseCloseID,
                20,
                20
            );
            if (this.ckplayerConfig["style"]["logo"]) {
                if (this.ckplayerConfig["style"]["logo"]["file"]) {
                    ...
                    if (
                        logoFile.substr(0, 15) == "data:image/png;" ||
                        logoFile.substr(0, 15) == "data:image/jpg;" ||
                        logoFile.substr(0, 16) == "data:image/jpeg;"
                    ) {
                        this.getByElement(logoID).innerHTML =
                            '<img src="' + logoFile + '" border="0">'; //构建logo
                    }
                }
            } else {
                this.getByElement(logoID).innerHTML =
                    this.vars["logo"] || this.logo || ""; //构建logo
            }
            var pd = this.PD;
            ...
            this.css(
                [dlineID + "-la", dlineID + "-lb", dlineID + "-lc"],
                "float",
                "left"
            );
            this.css(
            ...
            if (
                !this.ckplayerConfig["config"]["mobileVolumeBarShow"] &&
                this.isMobile()
            ) {
                this.css(
                    [muteID, escMuteID, volumeID, volumeDbgID, dlineID + "-re"],
                    {
                        display: "none",
                    }
                );
            ...
            this.addListenerInside(
                "mouseover",
                cPlayOver,
                this.getByElement(playID + "-canvas")
            );
            this.addListenerInside(
                "mouseout",
                cPlayOut,
            ...
            //暂停按钮
            var cPause = this.getByElement(pauseID + "-canvas").getContext(
                "2d"
            );
            ...
            this.addListenerInside(
                "mouseover",
                cPauseOver,
                this.getByElement(pauseID + "-canvas")
            );
            this.addListenerInside(
                "mouseout",
                cPauseOut,
            ...
            //前一集按钮
            var cFront = this.getByElement(frontID + "-canvas").getContext(
                "2d"
            );
            ...
            this.addListenerInside(
                "mouseover",
                cFrontOver,
                this.getByElement(frontID + "-canvas")
            );
            this.addListenerInside(
                "mouseout",
                cFrontOut,
            ...
            this.addListenerInside(
                "mouseover",
                cNextOver,
                this.getByElement(nextID + "-canvas")
            );
            this.addListenerInside(
                "mouseout",
                cNextOut,
            ...
            this.addListenerInside(
                "mouseover",
                cFullOver,
                this.getByElement(fullID + "-canvas")
            );
            this.addListenerInside(
                "mouseout",
                cFullOut,
            ...
            //定义退出全屏按钮样式
            var cEscFull = this.getByElement(escFullID + "-canvas").getContext(
                "2d"
            );
            ...
            this.addListenerInside(
                "mouseover",
                cEscFullOver,
                this.getByElement(escFullID + "-canvas")
            );
            this.addListenerInside(
                "mouseout",
                cEscFullOut,
            ...
            this.addListenerInside(
                "mouseover",
                cMuteOver,
                this.getByElement(muteID + "-canvas")
            );
            this.addListenerInside(
                "mouseout",
                cMuteOut,
            ...
            //定义取消广告静音按钮样式
            var cEscMute = this.getByElement(escMuteID + "-canvas").getContext(
                "2d"
            );
            ...
            this.addListenerInside(
                "mouseover",
                cEscMuteOver,
                this.getByElement(escMuteID + "-canvas")
            );
            this.addListenerInside(
                "mouseout",
                cEscMuteOut,
            ...
            //定义广告静音按钮的样式
            var cAdMute = this.getByElement(adMuteID + "-canvas").getContext(
                "2d"
            );
            ...
            this.addListenerInside(
                "mouseover",
                cAdMuteOver,
                this.getByElement(adMuteID + "-canvas")
            );
            this.addListenerInside(
                "mouseout",
                cAdMuteOut,
            ...
            //定义取消静音广告按钮样式
            var cAdEscMute = this.getByElement(
                adEscMuteID + "-canvas"
            ).getContext("2d");
            ...
            this.addListenerInside(
                "mouseover",
                cAdEscMuteOver,
                this.getByElement(adEscMuteID + "-canvas")
            );
            this.addListenerInside(
                "mouseout",
                cAdEscMuteOut,
            ...
            //定义暂停广告关闭按钮
            var adPauseClose = this.getByElement(
                adPauseCloseID + "-canvas"
            ).getContext("2d");
            ...
            this.addListenerInside(
                "mouseover",
                adPauseCloseOver,
                this.getByElement(adPauseCloseID + "-canvas")
            );
            this.addListenerInside(
                "mouseout",
                adPauseCloseOut,
            ...
            //定义loading样式
            var cLoading = this.getByElement(loadingID + "-canvas").getContext(
                "2d"
            );
            ...
            cLoadingFillRect();
            //定义中间暂停按钮的样式
            var cPauseCenter = this.getByElement(
                pauseCenterID + "-canvas"
            ).getContext("2d");
            ...
            this.addListenerInside(
                "mouseover",
                cPauseCenterOver,
                this.getByElement(pauseCenterID + "-canvas")
            );
            this.addListenerInside(
                "mouseout",
                cPauseCenterOut,
        ...
        addButtonEvent: function () {
            ...
            var frontClick = function (event) {
                if (thisTemp.vars["front"]) {
                    ...
                    thisTemp.sendJS(
                        "clickEvent",
                        "actionScript->" + thisTemp.vars["front"]
                    );
                }
            };
            ...
            var nextClick = function (event) {
                if (thisTemp.vars["next"]) {
                    ...
                    thisTemp.sendJS(
                        "clickEvent",
                        "actionScript->" + thisTemp.vars["next"]
                    );
                }
            };
            ...
            var adSkipClick = function (event) {
                if (
                    thisTemp.CB["adSkip"].innerHTML ==
                    thisTemp.language["skipAd"]
                ) {
            ...
            var volumePrompt = function (vol) {
                ...
                var promptObj = {
                    ...
                    x:
                        volumeBOXY["x"] +
                        thisTemp.CB["volumeBO"].offsetWidth * 0.5,
                    y: volumeBOXY["y"],
                };
        ...
        timeButtonMouseDown: function () {
            ...
            var timeBoBgmousemove = function (event) {
                ...
                if (!thisTemp.isUndefined(thisTemp.CB["definitionP"])) {
                    if (
                        thisTemp.css(thisTemp.CB["definitionP"], "display") !=
                        "block"
                    ) {
                        def = true;
                    }
        ...
        progressClick: function (obj) {
			...
			*/
            ...
            var referMouseClick = function (event) {
                var referX =
                    thisTemp.client(event)["x"] -
                    thisTemp.getCoor(obj["refer"])["x"];
                ...
                if (obj["fun"]) {
                    if (obj["grossValue"] === "time") {
                        var sliderXY = thisTemp.getCoor(
                            thisTemp.CB["timeButton"]
                        );
                        sliderLeft = sliderXY["x"];
                        if (
                            !thisTemp.checkSlideLeft(referX, sliderLeft, rWidth)
                        ) {
                            return;
                        }
                        var bimeButtonWB =
        ...
        slider: function (obj) {
			...
			*/
            ...
            var calculation = function () {
                ...
                var rWidth =
                    obj["refer"].offsetWidth - obj["slider"].offsetWidth;
                var grossValue = 0;
            ...
            var mMove = function (event) {
                ...
                if (obj["slider"] === thisTemp.CB["timeButton"]) {
                    if (
                        !thisTemp.checkSlideLeft(
                            newX,
                            sliderLeft,
                            obj["refer"].offsetWidth
                        )
                    ) {
                        return;
                    }
		...
		*/
        loadingStart: function (rot) {
            ...
            var loadingFun = function () {
                ...
                nowRotate = parseInt(
                    nowRotate.replace("rotate(", "").replace("deg);", "")
                );
                nowRotate += 4;
                if (nowRotate > 360) {
                    nowRotate = 0;
		...
		*/
        showFrontNext: function () {
            ...
            if (this.vars["front"]) {
                this.css(
                    [this.CB["front"], this.CB["frontLine"]],
                    "display",
                    "block"
                );
            } else {
                ...
                if (typeof cMenu["more"] == "object") {
                    if (cMenu["more"].length > 0) {
                        ...
                        for (i = 0; i < moreArr.length; i++) {
                            ...
                            if (
                                mTemp["clickEvent"] &&
                                mTemp["clickEvent"] != "none"
                            ) {
                                var eveObj = this.clickEvent(
                                    mTemp["clickEvent"]
                                );
        ...
        controlBarHide: function (hide) {
            ...
            var controlBarShow = function (show) {
                if (show && !cShow && thisTemp.controlBarIsShow) {
                    ...
                    thisTemp.css(
                        thisTemp.CB["controlBarBg"],
                        "display",
                        "block"
                    );
                    thisTemp.css(thisTemp.CB["controlBar"], "display", "block");
                    thisTemp.css(
                        thisTemp.CB["timeProgressBg"],
                        "display",
                        "block"
                    );
            ...
            var cbarFun = function (event) {
                if (
                    client["x"] == oldClient["x"] &&
                    client["y"] == oldClient["y"]
                ) {
            ...
            var keyDown = function (eve) {
                ...
                switch (keycode) {
                    ...
                    case 38:
                        now =
                            thisTemp.volume +
                            thisTemp.ckplayerConfig["config"]["volumeJump"];
		...
		*/
        playbackRate: function () {
            if (
                !this.showFace ||
                !this.ckplayerConfig["config"]["playbackRate"]
            ) {
                return;
            }
            var thisTemp = this;
            ...
            if (vArr.length > 1) {
                ...
                this.css(
                    [this.CB["playbackrate"], this.CB["playbackrateLine"]],
                    "display",
                    "block"
                );
                ...
                this.buttonWidth["playbackrate"] =
                    this.CB["playbackrate"].offsetWidth;
            } else {
        ...
        addPlaybackrate: function () {
            ...
            var defClick = function (event) {
                thisTemp.css(thisTemp.CB["playbackrateP"], {
                    left:
                        thisTemp.getCoor(thisTemp.CB["playbackrate"])["x"] +
                        "px",
                    display: "block",
                });
            };
		...
		*/
        subtitleSwitch: function () {
            ...
            if (vArr.length > 1) {
                ...
                this.css(
                    [this.CB["subtitles"], this.CB["subtitlesLine"]],
                    "display",
                    "block"
                );
                ...
                this.buttonWidth["subtitles"] =
                    this.CB["subtitles"].offsetWidth;
            } else {
        ...
        addSubtitles: function () {
            ...
            var defClick = function (event) {
                thisTemp.css(thisTemp.CB["subtitlesP"], {
                    left:
                        thisTemp.getCoor(thisTemp.CB["subtitles"])["x"] + "px",
                    display: "block",
                });
            };
		...
		*/
        definition: function () {
            if (
                !this.showFace ||
                !this.ckplayerConfig["config"]["definition"]
            ) {
                return;
            }
            var thisTemp = this;
            ...
            if (dArr.length > 1) {
                ...
                this.css(
                    [this.CB["definition"], this.CB["definitionLine"]],
                    "display",
                    "block"
                );
                ...
                this.buttonWidth["definition"] =
                    this.CB["definition"].offsetWidth;
            } else {
        ...
        addDefListener: function () {
            ...
            var defClick = function (event) {
                thisTemp.css(thisTemp.CB["definitionP"], {
                    left:
                        thisTemp.getCoor(thisTemp.CB["definition"])["x"] + "px",
                    display: "block",
                });
            };
        ...
        prompt: function () {
            ...
            var showPrompt = function (event) {
                ...
                var pW = pcon["pW"],
                    pT = pcon["pT"],
                    pL =
                        parseInt(thisTemp.css(this, "left")) -
                        parseInt(pW * 0.5);
                if (pcon["pL"] > 10) {
                    pL = pcon["pL"];
                }
                ...
                thisTemp.promptElement.innerHTML = thisTemp.getDataset(
                    this,
                    "words"
                );
                thisTemp.css(thisTemp.promptElement, {
		...
		*/
        changePrompt: function () {
            ...
            for (var i = 0; i < arr.length; i++) {
                ...
                var left =
                    parseInt((time * bw) / duration) -
                    parseInt(arr[i].offsetWidth * 0.5);
                if (left < 0) {
                    left = 0;
                }
		...
		*/
        preview: function (obj) {
                    ...
                    var loadImg = function (i) {
                        ...
                        img.onload = function (event) {
                            ...
                            if (thisTemp.previewDiv == null) {
                                ...
                                thisTemp.previewDiv =
                                    thisTemp.getByElement(random);
                                var eleTop =
                                    obj["y"] - parseInt(imgH * 0.1) + 2;
                                ...
                                thisTemp.PD.appendChild(ele2);
                                thisTemp.previewTop = thisTemp.getByElement(
                                    ele2.className
                                );
                            ...
                            var canvas = thisTemp.getByElement(
            ...
            if (this.previewStart == 2) {
                var isTween = true;
                var nowNum = parseInt(
                    obj["time"] / this.vars["preview"]["scale"]
                );
                var numTotal = parseInt(
                    thisTemp.getMetaDate()["duration"] /
                        this.vars["preview"]["scale"]
                );
                ...
                var imgWidth =
                    (thisTemp.previewDiv.offsetWidth * 0.01) / srcArr.length;
                var left =
                        imgWidth * nowNum - obj["x"] + parseInt(imgWidth * 0.5),
		...
		*/
        checkBarWidth: function () {
            ...
            var ele = [];
            ele.push([
                [this.CB["full"], this.CB["escFull"], this.CB["fullLine"]],
            ...
            if (
                ...
                ele.push([
                    [this.CB["mute"], this.CB["escMute"], this.CB["muteLine"]],
            ...
            ele.push([
                [this.CB["play"], this.CB["pause"], this.CB["playLine"]],
		...
		*/
        initPlayPause: function () {
            ...
            if (this.vars["autoplay"]) {
                this.css(
                    [this.CB["play"], this.CB["pauseCenter"]],
                    "display",
                    "none"
                );
		...
		*/
        ...
        loadAdPause: function () {
            ...
            if (this.isStrImage(type) && this.adPauseShow) {
                ...
                var imgHtml =
                    '<img src="' + ad["file"] + '" class="' + imgClass + '">';
                if (ad["link"]) {
                    imgHtml =
                        '<a href="' +
                        ad["link"] +
                ...
                this.addListenerInside(
                    ...
                    function () {
                        ...
                        imgObj.src = this.src;
                        var imgWH = thisTemp.adjustmentWH(
                            imgObj.width,
                            imgObj.height
                        );
                        thisTemp.css(
                        ...
                        if (
                            thisTemp.ckplayerConfig["style"]["advertisement"][
                                "closeButtonShow"
                            ] &&
		...
		*/
        ...
        advertisementsTime: function (nt) {
            ...
            if (this.adTimeAllTotal > 0) {
                this.CB["adTime"].innerHTML = this.language["adTime"].replace(
                    "{$second}",
        ...
        adSkipButtonShow: function () {
            ...
            var timeFun = function () {
                if (delayTimeTemp >= 0) {
                    thisTemp.CB["adSkip"].innerHTML = thisTemp.language[
                        "skipAdTime"
                    ].replace(
                        "{$second}",
        ...
        loadAdvertisements: function () {
            ...
            if (this.isStrImage(type)) {
                ...
                this.css(
                    [this.CB["adMute"], this.CB["adEscMute"]],
                    "display",
                    "none"
                );
                var imgClass = "adimg" + this.randomString(10);
            ...
            if (ad["link"]) {
                ...
                var link =
                    '<a href="' +
                    ad["link"] +
                    '" target="_blank" class="ckadmorelink">' +
                    this.language["adLink"] +
                    "</a>";
		...
		*/
        newAdOther: function (i) {
            ...
            if (
                this.ckplayerConfig["style"]["advertisement"][
                    "closeOtherButtonShow"
                ]
            ) {
                ...
                this.getByElement(closeAdDivID).innerHTML = this.newCanvas(
                    closeAdDivID,
                    20,
                    20
                );
                ...
                var adOtherClose = this.getByElement(
                    closeAdDivID + "-canvas"
                ).getContext("2d");
                ...
                this.addListenerInside(
                    "mouseover",
                    adOtherCloseOver,
                    this.getByElement(closeAdDivID + "-canvas")
                );
                this.addListenerInside(
                    "mouseout",
                    adOtherCloseOut,
                    ...
                    if (!this.isUndefined(ad["close"])) {
                        if (!ad["close"]) {
                            ...
                            var x = coor["x"],
                                ...
                                cx =
                                    x +
                                    this.getByElement(ad["div"]).offsetWidth -
                                    10,
                                cy = y - 10;
            ...
            if (!this.isUndefined(this.advertisements["inserttime"])) {
                ...
                for (var i = 0; i < arr.length; i++) {
                    if (arr[i].toString().substr(-1) == "%") {
                        newArr.push(
                            parseInt(duration * parseInt(arr[i]) * 0.01)
                        );
                    } else {
		...
		*/
        endedHandler: function () {
            ...
            if (
                !this.endAdPlay &&
                !this.isUndefined(this.advertisements["end"])
            ) {
                this.endAdPlay = true;
                this.adI = 0;
                this.adType = "end";
        ...
        bufferEdHandler: function () {
            ...
            var bufferFun = function () {
                if (
                    ...
                    var loadProgressBgW =
                        thisTemp.CB["timeProgressBg"].offsetWidth;
            ...
            if (!this.vars["live"]) {
                ...
                var timeFun = function () {
                    if (
                        ...
                        thisTemp.CB["timeText"].innerHTML =
                            thisTemp.getNowDate();
                    }
                };
		...
		*/
        changeVolume: function (vol, bg, button) {
            ...
            if (button && this.showFace) {
                ...
                if (
                    buLeft >
                    this.CB["volumeBg"].offsetWidth -
                        this.CB["volumeBO"].offsetWidth
                ) {
                    buLeft =
                        this.CB["volumeBg"].offsetWidth -
                        this.CB["volumeBO"].offsetWidth;
		...
		*/
        adEscMuteFunction: function () {
            ...
            var v =
                this.ckplayerConfig["style"]["advertisement"]["videoVolume"];
            this.changeVolume(v);
            this.adMuteInto();
        },
        /*
        ...
        changeConfig: function () {
            ...
            switch (arg.length) {
                ...
                case 5:
                    ...
                    break;
                case 6:
                    this.ckplayerConfig[arg[0]][arg[1]][arg[2]][arg[3]][
                        arg[4]
                    ] = val;
                    break;
                case 7:
                    this.ckplayerConfig[arg[0]][arg[1]][arg[2]][arg[3]][arg[4]][
                        arg[5]
                    ] = val;
                    break;
                case 8:
                    ...
                    for (i = 0; i < arr.length; i++) {
                        ...
                        if (arr2) {
                            if (f != "") {
                                ...
                                w += ",";
                                v["type"] +=
                                    this.ckplayerConfig["config"]["split"];
                            }
                            f += encodeURIComponent(
                                decodeURIComponent(arr2[0])
                            );
                            d += arr2[2];
                            w += arr2[3];
                ...
                switch (list[i]["type"]) {
                    ...
                    case "text":
                        ...
                        this.css(idArr[i], {
                            float: "left",
                            width:
                                this.getByElement(idArr[i] + "_text")
                                    .offsetWidth + "px",
                            height:
                                this.getByElement(idArr[i] + "_text")
                                    .offsetHeight + "px",
                        ...
                        this.css(idArr[i] + "_bg", {
                            ...
                            height:
                                this.getByElement(idArr[i] + "_text")
                                    .offsetHeight + "px",
                            filter:
                                "alpha(opacity:" + list[i]["backAlpha"] + ")",
                            opacity: list[i]["backAlpha"].toString(),
                            borderRadius: list[i]["backRadius"] + "px",
                            backgroundColor: list[i]["backgroundColor"].replace(
                                "0x",
                                "#"
                            ),
                            position: "absolute",
        ...
        calculationCoor: function (ele) {
            ...
            if (position.length > 0) {
                ...
                if (x.substring(x.length - 1, x.length) == "%") {
                    x = Math.floor(
                        parseInt(x.substring(0, x.length - 1)) * w * 0.01
                    );
                }
                if (y.substring(y.length - 1, y.length) == "%") {
                    y = Math.floor(
                        parseInt(y.substring(0, y.length - 1)) * h * 0.01
                    );
                }
            }
            return {
        ...
        changeElementCoor: function () {
            for (var i = 0; i < this.elementArr.length; i++) {
                if (this.getByElement(this.elementArr[i]) != []) {
                    var c = this.calculationCoor(
                        this.getByElement(this.elementArr[i])
                    );
                    if (c["x"] && c["y"]) {
            ...
            var Tween = {
                ...
                Bounce: {
                    ...
                    easeInOut: function (t, b, c, d) {
                        if (t < d / 2)
                            return (
                                Tween.Bounce.easeIn(t * 2, 0, c, d) * 0.5 + b
                            );
                        else
                            return (
		...
		*/
        animate: function (attribute) {
            ...
            switch (obj["parameter"]) {
                case "x":
                    if (obj["start"] == null) {
                        ...
                        if (
                            start.substring(start.length - 1, start.length) ==
                            "%"
                        ) {
            ...
            var callBack = function () {
                ...
                index = thisTemp.arrIndexOf(
                    thisTemp.animatePauseArray,
                    animateId
                );
                if (index > -1) {
		...
		*/
        ajax: function (cObj) {
            ...
            if (
                ...
                var params =
                    this.formatParams(obj.data) + "&callback=" + callbackName; //按时间戳拼接字符串
		...
		*/
        getNewUrl: function (url) {
            if (this.isContains(url, "?")) {
                return (url +=
                    "&" + this.randomString(8) + "=" + this.randomString(8));
            } else {
                return (url +=
                    "?" + this.randomString(8) + "=" + this.randomString(8));
            }
        ...
        timer: function (time, fun, number) {
            ...
            this.start = function () {
                if (!thisTemp.runing) {
                    ...
                    thisTemp.timeObj = window.setInterval(
                        thisTemp.startFun,
                        time
                    );
                }
