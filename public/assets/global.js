...
function pe_cfone(_this, show) {
	var _text = arguments[2] ? show : "确认"+show+'?';
	if (confirm(_text)) {
		if ($(_this).is("a")) {
			return true;
		}
...
function pe_countdown(id, etime) {
	setInterval(function(){
	    ...
	    if(myD>= 0){
			//var str = myD+"небо"+myH+"Час"+myM+"минута"+myS+"."+myMS+"Второй";
			var str = myD+"天"+myH+"小时"+myM+"分钟"+myS+"秒";
	    }else{
			var str = "0小时0分钟0秒";
		}
		obj.html(str);
	...
	$(obj).each(function(index){
		...
		setInterval(function(){
			...
			if (typeof(type) != 'undefined' && type == 'html') {
			    if (all_s >= 0) {
					_this.html(js_h+'小时'+js_m+'分钟'+js_s+'秒');	
			    }
			    else{
					_this.html('0小时0分钟0秒');
				}
