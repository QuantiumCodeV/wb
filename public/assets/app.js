/**
 * @copyright   2023 团购免税演示 <http://zhu.xxx.cn>
...
function app_getlist(url, event, func) {
    ...
    var sleep = 0;
    $("#getmore_jindu").html('<div id="getmore_load">загрузка...</div>').show();
    if (pageid >= 10) {
        //$("#getmore_jindu").show();
        sleep = 800;
    }
    var app_info =
        "&city=" + app_pageval("city") + "&location=" + app_getval("location");
    $.getJSON(
        url + "&page=" + page + "&pageid=" + pageid + app_info,
        {},
        function (json) {
            setTimeout(function () {
                ...
                if (json.result) {
                    ...
                    $("#json_html")
                        .clone()
                        .insertBefore("#json_html")
                        .attr("id", "json_html_" + page)
                        .find("#json_tpl")
                        .attr("id", "json_tpl_" + page);
                    $("#json_html_" + page).html(
                        template("json_tpl_" + page, json)
                    );
        ...
        navigator.geolocation.getCurrentPosition(
            function (position) {
                ...
                convertor.translate(pointArr, 3, 5, function (data) {
                    app_setval(
                        "location",
                        data.points[0].lng + "_" + data.points[0].lat
                    );
...
function app_paypw(url, func) {
    var arr = new Array();
    var paypw = window.prompt("Введите фонды пароль", "");
    if (paypw == "") {
        alert("Введите фонды пароль!");
        arr["result"] = false;
        return arr;
    }
    if (paypw == null) {
...
function app_iframe(url) {
    ...
    var html =
        '<div id="app_iframe" style="height:' +
        height +
        ";width:" +
        width +
        ';display:none"><iframe src="' +
        url +
        '" style="width:100%;height:100%;border:0"></iframe></div>';
    if (!$("#app_iframe_hide").is("div"))
        $("body").wrapInner('<div id="app_iframe_hide"></div>');
    $("body").append(html);
...
function app_page(id, func) {
    $("body").css("overflow-y", "hidden");
    $("#" + id).wrap(
        '<div class="app_page" style="position:relative; z-index:99;"></div>'
    );
    var _height =
    ...
    $("#" + id)
        ...
        .fadeIn(400)
        .addClass("app_pagemain");
    $("#" + id).before(
        '<div class="app_pagehide" style="position:fixed; background-color:rgba(0, 0, 0, 0.7); width:100%; height:100%; z-index:99;top:0;left:0" onclick="app_page_close()"></div>'
    );
    if (func && typeof func == "function") {
        func();
    }
